"""Search Agent with Strategy Optimization.

This agent is responsible for:
1. Selecting the optimal search strategy based on query characteristics
2. Executing searches using the appropriate tool (BM25, kNN, hybrid, etc.)
3. Evaluating result quality and adapting strategy if needed
4. Returning structured, ranked results with strategy metadata

Note: Query validation and medical reasoning are handled by other agents:
- Orchestrator: Query understanding, validation, refinement
- Synthesis Agent: Medical interpretation and reasoning
"""
import logging
